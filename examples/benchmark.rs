...
mod internal {
    ...
    fn benchmark_route_generation_with_count(custom_airport_count: Option<usize>) {
        ...
        .unwrap_or_else(|| {
            eprintln!("Failed to initialize data");
            (
                Arc::new(RouteGenerator::new(
                    Vec::new(),
        ...
        if route_generator.all_airports.is_empty() && aircraft.is_empty() {
            return; // Initializer failed
        }
...
fn main() {
    ...
    internal::run();
    #[cfg(not(feature = "gui"))]
    println!("Benchmark requires feature 'gui' to be enabled.");
}
