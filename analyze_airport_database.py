...
def generate_rust_code(cursor):
    ...
    _generate_airport_constants(cursor)
    _generate_elevation_constants(cursor)
    _generate_geographic_constants(cursor)
    _generate_runway_count_distribution(cursor)
    _generate_runway_length_constants(cursor)
    _generate_runway_width_constants(cursor)
    _generate_surface_type_distribution(cursor)


def _generate_airport_constants(cursor):
    """Generate airport count constant."""
    cursor.execute("SELECT COUNT(*) FROM airports")
    airport_count = cursor.fetchone()[0]
...
def _generate_elevation_constants(cursor):
    """Generate elevation distribution constants."""
...
def _generate_geographic_constants(cursor):
    """Generate geographic distribution constants."""
    cursor.execute("""
        SELECT
            MIN(Latitude) as min_lat,
            MAX(Latitude) as max_lat,
...
def _generate_runway_count_distribution(cursor):
    """Generate runway count distribution array."""
    cursor.execute("""
        SELECT
            runway_count,
            CAST(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT AirportID) FROM runways) AS REAL) as percentage
...
def _generate_runway_length_constants(cursor):
    """Generate runway length distribution constants."""
    ...
    for p in [0.25, 0.50, 0.75, 0.90]:
        length_percentiles[p] = get_percentile(
            cursor, "runways", "Length", p, "Length > 0"
        )
...
def _generate_runway_width_constants(cursor):
    """Generate runway width distribution constants."""
    cursor.execute("SELECT MIN(Width), MAX(Width) FROM runways WHERE Width > 0")
    min_width, max_width = cursor.fetchone()

    width_percentiles = {}
    for p in [0.25, 0.50, 0.75, 0.90]:
        width_percentiles[p] = get_percentile(
            cursor, "runways", "Width", p, "Width > 0"
        )
...
def _generate_surface_type_distribution(cursor):
    """Generate surface type distribution array."""
    cursor.execute("""
        SELECT
            Surface,
            CAST(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM runways) AS REAL) as percentage
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if platform.system() == "Windows":
            # Windows: %APPDATA%\flight-planner\airports.db3
            db_path = home / "AppData" / "Roaming" / "flight-planner" / "airports.db3"
        else:
            # Linux/macOS: ~/.local/share/flight-planner/airports.db3
