...
1.  **Format:** Run `make fmt` (or `cargo fmt`) to ensure code style compliance.
2.  **Lint:** Run `make clippy` (or `cargo clippy`) to catch common mistakes and improve code quality.
3.  **Test:** Run `make test` (or `cargo test`) to ensure all tests pass. **YOU MUST RUN THIS AND ENSURE IT PASSES BEFORE COMMITTING.**
